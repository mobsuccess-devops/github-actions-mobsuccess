...
describe("genaiDetection", () => {
  ...
  describe("AUTOMATION_BOT_PATTERNS", () => {
    test.each([
      "github-actions[bot]",
      "dependabot[bot]",
      "renovate[bot]",
      ...
      "vercel[bot]",
      "mergify[bot]",
    ])("should match automation bot: %s", (author) => {
      expect(matchesAnyPattern(author, AUTOMATION_BOT_PATTERNS)).not.toBeNull();
    });
  });
  ...
  describe("detectGenAI", () => {
    ...
    test("should skip detection if already labeled with genai-assisted", async () => {
      ...
      expect(result.detected).toBe(false);
      expect(result.hasGenAiLabel).toBe(true);
    });
