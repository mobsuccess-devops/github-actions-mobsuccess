...
function hasRapidCommits(commits) {
  ...
  if (rapidPairs >= 2) {
    return {
      detected: true,
      details: `${rapidPairs + 1} commits within 30 seconds: ${[
        ...new Set(rapidCommits),
      ].join(", ")}`,
    };
  }
...
async function detectGenAI(octokit, { owner, repo, pullNumber, pullRequest }) {
  if (hasGenAILabel(pullRequest.labels)) {
    ...
    return { detected: false, reason: null, hasGenAiLabel: true };
  }
      ...
      if (commitAuthorMatch) {
        return {
          ...
          reason: `Commit "${commit.sha.slice(
            0,
            7
          )}" author "${commitAuthor}" matches bot pattern: ${commitAuthorMatch}`,
        };
      }
      ...
      if (committerMatch) {
        return {
          ...
          reason: `Commit "${commit.sha.slice(
            0,
            7
          )}" committer "${committer}" matches bot pattern: ${committerMatch}`,
        };
      }
    }
