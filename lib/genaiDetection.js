...
function hasRapidCommits(commits) {
  ...
  if (rapidPairs >= 3) {
    ...
    return {
      ...
      } commits within 15 seconds: ${uniqueRapidCommits.join(", ")}`,
    };
  }
...
// Detection options - some detections are disabled by default as they are "flags"
// that may need multiple signals to be confident. These will be enabled in v2
// with a multi-flag scoring system (e.g., label only if 2+ flags are detected).
// When enabled, detection results will include sub-flags to indicate which signals
// were detected, allowing more granular labeling.
const DEFAULT_OPTIONS = {
  detectRapidCommits: false, // v2: Enable as part of multi-flag scoring system
  detectCommentPatterns: false, // v2: Enable as part of multi-flag scoring system
};

async function detectGenAI(
  octokit,
